...
- [~] **G12** 🟡 · Documentation · [BI] PR #25 รอ review — ✋ [`docs/metric-dictionary.md`](metric-dictionary.md) skeleton: นิยามที่มีในโค้ดจริง (FY, timeliness, ERP fields, publication flag/SDG/national-international/dedupe rule ของ view, track_evaluation MERGE key) พร้อมสถานะ ✅/🟡/🔒 ต่อ entry · 🔒 เครื่องมือ dashboard + refresh schedule + ยืนยันนิยามจริง รอ PD-6/Research
- [~] **G8** 🟠 · Security (RBAC/PII) · [Security + BI] PR #25 รอ review — ✋ [`docs/data-classification.md`](data-classification.md): ระดับ P/I/PII/S ต่อ column ทุกตาราง (พบ `erp_2025.details` มีชื่อบุคคลจริง → PII; `reward/score` = S), กฎที่ใช้ได้ทันที (view เท่านั้น, n<5), แผน authorized view/policy tag/RBAC · 🔒 บังคับใช้จริงรอ PD-6
...
- [~] **G22** 🟢 · Security / CI · [DevOps] PR #24 รอ review — `.github/workflows/secret-scan-full.yml`: ทุกวันจันทร์ 08:00 ไทย + `workflow_dispatch`, `fetch-depth 0` + fetch ทุก branch/tag, gitleaks v8.30.1 (pinned) `--log-opts=--all`, exit 1 เมื่อพบ, SARIF report เป็น artifact 30 วัน · รันคำสั่งเดียวกันในเครื่องก่อน push = 0 leaks
